...
class FMFirstOrderLinear(nn.Module):
    ...
    def embed_float_fields(self, float_fields, embed=True):
        ...
        #float_embedding = torch.sum(float_embedding, dim=1, keepdim=True)  # 修改点1
    ...
    def embed_token_fields(self, token_fields):
        ...
        # [batch_size, 1, output_dim]
        #token_embedding = torch.sum(token_embedding, dim=1, keepdim=True)  # 修改点2
    ...
    def forward(self, interaction, index):  # 修改点0
        ...
        final_fields_embedding = torch.cat(total_fields_embedding, dim=1)

        #'''
        if not index == -1:
            batch_size = final_fields_embedding.shape[0]
            index_ = torch.full((batch_size, 1, self.output_dim), fill_value=index, dtype=torch.int64).to(device=self.device)
            value_ = torch.zeros(batch_size, 1, self.output_dim).to(device=self.device)
            emb_change = final_fields_embedding.scatter(1, index_, value_)  # [B,L,E]
            final_fields_embedding = emb_change
        #'''

        return torch.sum(final_fields_embedding, dim=1) + self.bias  # [batch_size, output_dim]

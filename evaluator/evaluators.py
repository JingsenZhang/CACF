...
from evaluator.abstract_evaluator import GroupedEvaluator, IndividualEvaluator
...
class TopKEvaluator(GroupedEvaluator):
    ...
    def evaluate(self, batch_matrix_list, eval_data):
        ...
        for k in self.topk:
            key = 'f1@{}'.format(k)
            P = float(metric_dict['precision@{}'.format(k)])
            R = float(metric_dict['recall@{}'.format(k)])

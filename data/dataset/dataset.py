...
class Dataset(object):
    ...
    imputation. Features are stored as :class:`pandas.DataFrame` inside :class:`~cacf.data.dataset.dataset.Dataset`.
    General and Context-aware Models can use this class.

    By calling method :meth:`~cacf.data.dataset.dataset.Dataset.build()`, it will processing dataset into
    DataLoaders, according to :class:`~cacf.config.eval_setting.EvalSetting`.
    ...
    Attributes:
        ...
        field2type (dict): Dict mapping feature name (str) to its type (:class:`~cacf.utils.enum_type.FeatureType`).

        field2source (dict): Dict mapping feature name (str) to its source
            (:class:`~cacf.utils.enum_type.FeatureSource`).
    ...
    def _data_filtering(self):
        ...
        Note:
            ...
            thus :meth:`~cacf.data.dataset.dataset.Dataset._reset_index` will reset the index of feats.
        """
    ...
    def _load_user_or_item_feat(self, token, dataset_path, source, field_name):
        ...
        Note:
            ``user_id`` and ``item_id`` has source :obj:`~cacf.utils.enum_type.FeatureSource.USER_ID` and
            :obj:`~cacf.utils.enum_type.FeatureSource.ITEM_ID`
        """
    ...
    def _fill_nan(self):
        ...
        For fields with type :obj:`~cacf.utils.enum_type.FeatureType.TOKEN`, missing value will be filled by
        ``[PAD]``, which indexed as 0.

        For fields with type :obj:`~cacf.utils.enum_type.FeatureType.FLOAT`, missing value will be filled by
    ...
    def float_like_fields(self):
        """Get fields of type :obj:`~cacf.utils.enum_type.FeatureType.FLOAT` and
        :obj:`~cacf.utils.enum_type.FeatureType.FLOAT_SEQ`.
    ...
    def token_like_fields(self):
        """Get fields of type :obj:`~cacf.utils.enum_type.FeatureType.TOKEN` and
        :obj:`~cacf.utils.enum_type.FeatureType.TOKEN_SEQ`.
    ...
    @property
    def seq_fields(self):
        """Get fields of type :obj:`~cacf.utils.enum_type.FeatureType.TOKEN_SEQ` and
        :obj:`~cacf.utils.enum_type.FeatureType.FLOAT_SEQ`.
    ...
    def non_seq_fields(self):
        """Get fields of type :obj:`~cacf.utils.enum_type.FeatureType.TOKEN` and
        :obj:`~cacf.utils.enum_type.FeatureType.FLOAT`.
    ...
    def build(self, eval_setting):
        ...
        See :class:`~cacf.config.eval_setting.EvalSetting` for details.
    ...
    def _dataframe_to_interaction(self, data):
        """Convert :class:`pandas.DataFrame` to :class:`~cacf.data.interaction.Interaction`.
